...
function Profile({ profile, setProfile, notify, userId, user, setMe }: any) {
  ...
  async function add(event: FormEvent) { event.preventDefault(); try { await api(editingSkillId ? '/api/perfil/habilidades/' + editingSkillId : '/api/perfil/habilidades', { method: editingSkillId ? 'PUT' : 'POST', body: JSON.stringify({ ...skill, confidence: Number(skill.confidence) }) }); setProfile(await api('/api/perfil')); setSkill({ topic: '', type: 'Domina', confidence: 4, visible: true }); setEditingSkillId(null); notify(editingSkillId ? 'Conocimiento actualizado.' : 'Tema agregado a tu perfil.'); } catch (error) { notify(error instanceof Error ? error.message : 'No pudimos guardar el tema.'); } }
  function editSkill(item: any) { setEditingSkillId(item.id); setSkill({ topic: item.topic, type: item.type === 0 ? 'Domina' : item.type === 1 ? 'NecesitaApoyo' : item.type, confidence: item.confidence ?? 4, visible: item.visible ?? true }); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  async function deleteSkill() { if (!pendingSkillDelete) return; const item = pendingSkillDelete; setPendingSkillDelete(null); try { await api('/api/perfil/habilidades/' + item.id, { method: 'DELETE' }); setProfile(await api('/api/perfil')); if (editingSkillId === item.id) { setEditingSkillId(null); setSkill({ topic: '', type: 'Domina', confidence: 4, visible: true }); } notify('Tema eliminado de tu mapa.'); } catch (error) { notify(error instanceof Error ? error.message : 'No pudimos eliminar el tema.'); } }
