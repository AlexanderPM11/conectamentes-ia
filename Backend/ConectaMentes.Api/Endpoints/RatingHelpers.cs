...
public static class RatingHelpers
{
    public static IQueryable<ReputationRow> ReputationRows(ConectaMentesDbContext db)
    {
        ...
        var connectionRows =
            from rating in db.Ratings
            join connection in db.Connections on rating.ConnectionId equals connection.Id
            join request in db.SupportRequests on connection.RequestId equals request.Id into requestGroup
            from request in requestGroup.DefaultIfEmpty()
            join user in db.Users on rating.EvaluatedUserId equals user.Id
            select new ReputationRow(rating.EvaluatedUserId, user.DisplayName, user.Career, request == null ? "Conexión directa" : request.Topic, rating.Usefulness, rating.Respect, rating.Fulfillment, rating.Clarity, (rating.Usefulness + rating.Respect + rating.Fulfillment + rating.Clarity) / 4d, rating.Comment, rating.CreatedAt);

        return sessionRows.Concat(connectionRows);
    }
}
